...
take image before yarn install
...
```bash
docker build -t cyphernode/batcher:v0.2.1-local .
...
```bash
docker build -t cyphernode/batcher:v0.2.1-local . ; docker stop `docker ps -q -f "name=batcher"`
```
